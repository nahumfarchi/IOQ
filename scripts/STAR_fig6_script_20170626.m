...
print_header(title1)
[R1, S1, periods1, period_is_fixed1, thetas1, ~, ~, EV1, EF1, FE1] = nrosy_wrapper(fp, fids_list, v0, N);
E1 = E_MIQ(m, thetas1, r, periods1, N);
DF1 = angles_to_DF(thetas1, local_frames, N);
disp(['E1 = ', num2str(E1)])
toc
...
%working
%local_frames = [repmat([1, 0, 0], m.nF, 1); repmat([0, 1, 0], m.nF, 1)];
%r = zeros(m.nE, 1);
%v0 = local_angles_to_gvectors(m, tmp_thetas, local_frames, 1);

%[local_frames2, r2] = create_local_frames(m);
%local_frames2 = local_frames1;
%r2 = r1;
print_header(title2)
[A2, b2, thetas2, periods2, theta_tags2, period_tags2] = create_MIQ_system(m, N, fids_list, v0, local_frames, r); 
...
DF2 = angles_to_DF(thetas2, local_frames, N);
disp(['E2 = ', num2str(E2)])
...
DF3 = angles_to_DF(unwrapped, local_frames, N);
E3 = E_MIQ(m, unwrapped, r, periods1, N);
disp(['E3 = ', num2str(E3)])
...
cvx_begin
    variable x5(m.nE)
    %variable k5(m.nE) integer
    %minimize( norm(x5, 2) + lambda*pi/2*norm(d0'*x5+get_gaussian_curvature(m), 1) )
    minimize( norm(x5(nonboundary_edge_ids), 2) + ...
              lambda*norm(d0(nonboundary_edge_ids, nonboundary_vert_ids)'*x5(nonboundary_edge_ids), 1) )
    subject to
        C*x5 == bc
        %d0(nonboundary_edge_ids, :)'*x5 = k(non
cvx_end

[DF5, thetas5] = create_direction_field(m, x5, fids_list(1), thetas_list(1), N, local_frames, r);
E5 = norm(x5)^2 + norm(d0'*x5,1);
title5 = ['|x|_2^2 + |d_0^Tx+G_k|_1 = ', num2str(norm(x5)^2), ' + ', num2str(norm(d0'*x5,1)), ' = ', num2str(E5)];
toc
...
hold on
m.draw(ones(m.nV, 1), 'EdgeAlpha', 1.0, 'FaceAlpha', 0.0)
draw_local_frames(m, local_frames)
m.draw(ones(m.nV, 1), 'FaceAlpha', 0.0, 'EdgeColor', period_tags2 > -1)
title('Local Frames'); hold off; view([0,0,1]); axis off
hold off
