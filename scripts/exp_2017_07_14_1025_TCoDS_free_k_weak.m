function exp_2017_07_14_1025_TCoDS_free_k_weak()
...
for fname = MESHES
    ...
    % Leave only last 2 constraints (Gauss-Bonnet and sum(u)=0)
    C = C(end-1:end,:);
    ...
    %B = 2*[(d0'*d0)*(d0'*d0), zeros(m.nV);
    %     zeros(m.nV), zeros(m.nV);
    %     (d0'*d0)*(d0'*d0), -(pi/2)*(d0'*d0)*(d0'*d0);
    %     -(pi/2)*(d0'*d0), -(pi/2)^2*eye(m.nV)];
    
    B = 2*[(d0'*d0)*(d0'*d0), zeros(m.nV);
         zeros(m.nV), zeros(m.nV)];
    B = B + 2*[(d0'*d0)*(d0'*d0), -(pi/2)*(d0'*d0)*(d0'*d0);
               -(pi/2)*(d0'*d0), -(pi/2)^2*eye(m.nV)];
    bb = -[d0'*d0; -(pi/2)*eye(m.nV)]*Ad;
    ...
    %'CheckGradients',true, 'SpecifyObjectiveGradient',true, 'SpecifyConstraintGradient',false);
    %fmincon(f, zeros(size(A,2),1), [], [], [], [], [], [], fcon, options);
    check_grad(@(x) E_weak(x, d0, Ad, 0.5), fcon, size(A,2));
    
    [A, ba, B, bb] = E_weak_matrices(d0, Ad);
    ...
    %idx_to_round = [2*m.nV];
    % Solve
    x = CoMISo_wrapper(full(C), full(B), bb, idx_to_round, true); % TODO add option for sparse matrix input to wrapper function
    u = x(1:m.nV);
    k = x(m.nV+1:end);
    connection = d0*u;
...
%     print(pngname, '-dpng', '-r0')
    ...
    function [f_, g_] = E_weak(x_, d0_, Ad_, la_)      
        if nargin < 4
            la_ = 1;
        end
        
        [A_, ba_, B_, bb_] = E_weak_matrices(d0_, Ad_, la_);
        ...
        %n_ = length(x_)/2;
        %d0d0_ = d0_'*d0_;
        %B_ = [d0d0_, (-pi/2)*eye(n_)];
        %bb_ = Ad_;
        %u_ = x_(1:n_);
        %k_ = x_(n_+1:end);
        
        %f2_ = norm(d0_*u_)^2 + norm(k_)^2 + norm(B_*x_ + bb_)^2;
        
        %g_ = 2*B_'*(B_*x_+bb_);
        %g_(1:n_) = g_(1:n_) + 2*d0d0_*u_;
        %g_(n_+1:end) = g_(n_+1:end) + 2*k_;

    end

    function [A_, ba_, B_, bb_] = E_weak_matrices(d0_, Ad_, la_)
        if nargin < 3
            la_ = 1;
        end
        ...
        d0d0_ = d0_'*d0_;
        n_ = length(Ad_);
        A_ = [d0_, zeros(size(d0_)); 
              zeros(n_), eye(n_);
              d0d0_, -la_*(pi/2)*eye(n_)];
        ba_ = -[zeros(size(d0_,1),1);
                zeros(n_, 1);
                la_*Ad_];       
        B3 = [d0d0_, (-pi/2)*eye(n_)];
        B_ = [2*d0d0_, zeros(n_); zeros(n_,n_), 2*eye(n_)] + 2*la_*(B3'*B3);
        bb_ = -2*la_*B3'*Ad_;
    end
