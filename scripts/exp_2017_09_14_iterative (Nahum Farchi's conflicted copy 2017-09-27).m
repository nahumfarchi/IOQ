function [] = exp_2017_08_31_1145_iterative()
    ...
    NUMBER_OF_SINGULARITIES.run = true;
    
    MIQ_VS_OTC_BUNNIES_LARGE.name = ...
        'MIQ_vs_OTC_bunnies_large_4gb_gpu';
    MIQ_VS_OTC_BUNNIES_LARGE.description = ...
        'MIQ vs OTC (cpu inv, gpu loop) vs OTC (cpu inv, cpu loop) vs OTC (block inv, gpu loop) on bunnies_large.';
    MIQ_VS_OTC_BUNNIES_LARGE.run = false;
    MIQ_VS_OTC_BUNNIES_LARGE.merge = false;
    MIQ_VS_OTC_BUNNIES_LARGE.plot = false;
    
    opt_vanilla = struct('inv_gpu', false, 'iter_gpu', false, 'TCODS', true);
    opt_inv_cpu = struct('inv_gpu', false, 'CUDAKernel_reduce_cols', true, 'TCODS', true);
    opt_block_inv_gpu = struct('block_inv_gpu', true, 'CUDAKernel_reduce_cols', true, 'TCODS', true);
    opt_inv_gpu = struct('inv_gpu', true, 'CUDAKernel_reduce_cols', true, 'TCODS', true);
    experiments = {
        struct(...
            'func', @run_lattice_iterative, ...
...
%         fig1 = figure();
%         n_faces = cellfun(@(x) x.nF, results(:,1));
%         [n_faces, inds] = sort(n_faces);
%         results = results(inds, :);
%         plot(n_faces, cellfun(@(x) x.E, results(:,1)), '--.', ...
%             n_faces, cellfun(@(x) x.E, results(:,2)), '--.')
%         legend('Lattice iter', 'MIQ', 'Location', 'northwest')
%         title('Energy')
% 
%         fig2 = figure();
%         n_faces = cellfun(@(x) x.nF, results(:,1));
%         plot(n_faces, cellfun(@(x) x.elapsed_total, results(:,1)), '--.', ...
%             n_faces, cellfun(@(x) x.elapsed, results(:,2)), '--.')
%         legend('Lattice iter', 'MIQ', 'Location', 'northwest')
%         title('Elapsed time (total)')
% 
%         fig3 = figure();
%         n_faces = cellfun(@(x) x.nF, results(:,1));
%         plot(n_faces, cellfun(@(x) x.elapsed_setup_lap, results(:,1)), '--.', ...
%             n_faces, cellfun(@(x) x.elapsed_setup, results(:,1)), '--.', ...
%             n_faces, cellfun(@(x) x.elapsed_inv, results(:,1)), '--.', ...
%             n_faces, cellfun(@(x) x.elapsed_iter, results(:,1)), '--.', ...
%             n_faces, cellfun(@(x) x.elapsed_total, results(:,1)), '--.')
%         legend('L', 'setup', 'inv', 'iter', 'total', 'Location', 'northwest')
%         title('Elapsed time (breakdown)')
% 
%         fig4 = figure();
%         n_faces = cellfun(@(x) x.nF, results(:,1));
%         plot(n_faces, cellfun(@(x) x.elapsed_total-x.elapsed_setup_lap, results(:,1)), '--.', ...
%             n_faces, cellfun(@(x) x.elapsed, results(:,2)), '--.')
%         legend('Lattice iter', 'MIQ', 'Location', 'northwest')
%         title('Elapsed time (total - L)')
% 
%         out_folder = get_experiment_folder(experiment_name);
%         mysave(fig1, 'energy', out_folder)
%         mysave(fig2, 'timing', out_folder)
%         mysave(fig3, 'timing_breakdown', out_folder);
%         mysave(fig4, 'timing_without_L', out_folder);
                                                    
    end
    
    if NUMBER_OF_SINGULARITIES.run
        number_of_singularities(1:10:100)
    end
    
function number_of_singularities(sing_x)
    global LOG;
    ...
    for n_singularities = sing_x
        ...
        res{i} = run_lattice_iterative(m, F0, THETA0, DEGREE, N_ITER, opt_block_inv_gpu);
        i = i + 1;
    end
...
function test_CUDA(fp)
    ...
    for mname = mesh_filenames
        ...
        res_CUDA(r) = run_lattice_iterative(mesh, ...
            F0, ...
            THETA0, ...
            DEGREE, ...
            N_ITER, ...
        ...
        opt.inv_gpu = false;
        res_chol(r) = run_lattice_iterative(mesh, ...
            F0, ...
            THETA0, ...
            DEGREE, ...
            N_ITER, ...
        ...
        res_CUDA_reduce_cols(r) = run_lattice_iterative(mesh, ...
            F0, ...
            THETA0, ...
            DEGREE, ...
            N_ITER, ...
    ...
    res_plot_cot_lap = run_lattice_iterative(mesh, ...
        F0, ...
        THETA0, ...
        DEGREE, ...
        N_ITER, ...
    ...
    opt.graph = true;
    res_plot_graph_lap = run_lattice_iterative(mesh, ...
        F0, ...
        THETA0, ...
        DEGREE, ...
        N_ITER, ...
...
function plot_results(plots, results, legends, out_folder)
    ...
    for i = 1:numel(plots)
        ...
        for j = 1:cols
            yy = results(:, j);
            valid = cellfun(@(x) ~isempty(x) & isfield(x, 'failed'), yy);
...
function cpu_inv_VS_MIQ()
    ...
    for mname = mesh_filenames
        ...
        opt.TCODS = true;
        try
            res_ter = run_lattice_iterative(mesh, ...
                F0, ...
                THETA0, ...
                DEGREE, ...
                N_ITER, ...
