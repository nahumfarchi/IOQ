classdef (Sealed) Options < handle %matlab.mixin.Copyable
    ...
    properties
        ...
        % Setup - elimination
        %-------------------------
        elimination = true                      % Use elimination levels; if false, only aggregation levels are constructed
        ...
        numCycles = 20                          % max #cycles to run
        initialGuessNorm = 1e+0                 % Initial guess magnitude
        finalErrorNorm = 1e-15                  % Stop when this error norm is reached
        errorReductionTol = 1e-7 %1e-8          % Terminate when error norm is reduced by this factor
