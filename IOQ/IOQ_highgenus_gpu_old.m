function [alpha_P, beta_P, elapsed_total, Lp, out] = ...
    ...
    addOptional(p, 'JLEps', 0.5);
    addOptional(p, 'JLFac', 24);
    ...
    if genus == 0
        ...
        ng2 = 2*genus;
        %[~, K, d0, ~, H] = tcods_gsystem(verts, faces);
        if ~exist('d0', 'var')
...
function x = gridsearch(M, x, x0, opt)
    ...
    %M_tilde = gpuArray(single(A'*M*A));
    %b = gpuArray(single(2*A'*M*(A*x - x0)));
    %dm = gpuArray(single(diag(M_tilde)));
    if opt.UseGPU
        M_tilde = gpuArray(single(M));
    ...
    for iter = 1:opt.Iterations
        if opt.Histories
            if genus > 0
                ...
                E_hist1(end+1) = gather((x-x0)'*M_tilde*(x-x0));
            end
        end
        ...
        if (opt.UseGPU && opt.bsx) || ~opt.UseGPU
            ...
            i = i(j);
        elseif opt.kernel
            [out_min, out_r] = feval(kernel, out_min, out_r, M_tilde, dm, b, nv, nv, num_elem);
            out_r = out_r + 1;
