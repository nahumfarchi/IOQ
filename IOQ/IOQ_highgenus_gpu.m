function [alpha_P, beta_P, elapsed_total, Lp, out] = ...
	...
	if isempty(opt.alpha_P)
        ...
        out.init_alpha = alpha_P;
		out.n_init_sing = length(inds);
	else
		alpha_P = opt.alpha_P;
		assert(length(alpha_P) == nv)
	end
    ...
    min_val = inf;
    x = alpha_P;
    if strcmpi(opt.InvMethod, 'ApproxResistance')
        tic
        alpha_P = gridsearch_approx_res(Rtilde, F, Ztilde, x, x0, opt);
        alpha_P = gather(double(alpha_P));
        if genus == 0
            beta_P = [];
        else
            A2 = (H'*d0) / F;
            y0 = (2/pi)*(A2*alpha_G - beta_G);
            beta_P = round(A2*alpha_P - y0);
        end
        alpha_P = gather(double(alpha_P));
        beta_P = gather(double(beta_P));
        if ~isempty(gd), wait(gd); end; elapsed_total = elapsed_total + toc;
    elseif genus == 0
        ...
        %if strcmpi(opt.highg_method, 'option1a')
                ...
                if strcmpi(opt.InvMethod, 'ApproxResistance')
                    error('?')
                    %x = gridsearch_approx_res(Rtilde, F, Ztilde, x, x0, opt);
                else
    ...
    %aa = F \ (alpha_G - (pi/2)*alpha_P);
    %bb = factorize(H'*B) \ (beta_G-(pi/2)*beta_P - H'*d0*aa);
    %fprintf('Eioq = %.4g\n', norm(d0*aa)^2 + norm(B*bb)^2);
...
function x = gridsearch_approx_res(Rtilde, F, Ztilde, x, x0, opt)
    ...
    for iter = 1:opt.Iterations
        ...
        if opt.Histories
            ...
            E_hist1(end+1) =  gather(0.5 * (x-x0)' * btilde_cpu);
        end
        ...
        elseif opt.Kernel && opt.UseGPU
            ...
            [min_val, i] = min(bsxfun(@plus, btilde_cpu, -btilde_cpu') + squareform(Rtilde));
...
function x = gridsearch(M, x, x0, opt)
    ...
    if opt.UseGPU
        try
            ...
            disp('Out of memory on GPU. Trying to use lower triangle part of the resistance distance')
            if ~exist('b', 'var')
                b = single(2*M*(x - x0));
            end
            if ~exist('dm', 'var')
...
%         b = gpuArray(single(zeros(nv, 1)));
%         [min_val2, i2] = min(bsxfun(@plus, dm+b, (dm-b)') - 2*M);
%         [min_val3, j2] = min(min_val2);
%         i2 = i2(j2);
%         
%         [min_val4, i4] = min(squareform(Mtril));
%         [min_val5, j5] = min(min_val4);
%         i5 = i4(j5);
        ...
        elseif opt.Kernel
            switch opt.KernelEntry
                ...
                case 'reduce_cols_R_symmetric'
                    [out_min, out_r] = feval(kernel, out_min, out_r, Mtril, b, nv, nv, num_elem);       
                otherwise
...
function [x] = gridsearch2(M1, x, x0, M2, A2, y, y0, opt)
    ...
    if opt.UseGPU
        ...
        M2_tilde = single(gpuArray(full(A2'*M2*A2)));
