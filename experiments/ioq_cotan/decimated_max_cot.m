...
E1 = ((2/pi)*alpha1 - alpha_g)' * Lc * ((2/pi)*alpha1 - alpha_g);
...
[alpha2, beta2, elapsed_ioq2] = IOQ_highgenus_gpu(...
    ...
    'Laplacian', 'cot', ...
    'InvMethod', INV_METHOD);
E2 = ((2/pi)*alpha2 - alpha_g)' * Lc * ((2/pi)*alpha2 - alpha_g);
...
title_ioq = {'IOQ', sprintf('$E = %g, |S| = %d$',           res_ioq_conn.miq_energy,   res_ioq_conn.n_vert_sing)};
title_ioq_cot = {'IOQc', sprintf('$E = %g, |S| = %d$',   res_ioq_cot.miq_energy,    res_ioq_cot.n_vert_sing)};
title_ioq_f = {'IOQe', sprintf('$E = %g, |S| = %d$',         res_ioq_conn_f.miq_energy, res_ioq_conn_f.n_vert_sing)};
title_ioq_cot_f = {'IOQe cot', sprintf('$E = %g, |S| = %d$', res_ioq_cot_f.miq_energy,  res_ioq_cot_f.n_vert_sing)};
title_miq = {'MIQ', sprintf('$E = %g, |S| = %d$',           res_miq.miq_energy,   res_ioq_conn.n_vert_sing)};
...
figure
subplot(221); res_ioq_conn.draw(props{:})    ; title(title_ioq);
subplot(222); res_ioq_cot.draw(props{:})     ; title(title_ioq_cot) ;
subplot(223); res_ioq_conn_f.draw(props{:})  ; title(title_ioq_f) ; 
subplot(224); res_ioq_cot_f.draw(props{:})   ; title(title_ioq_cot_f) ;
...
figure
filename = fullfile(out_folder, [meshname, '_conn.pdf']);
...
figure
filename = fullfile(out_folder, [meshname, '_cot.pdf']);
names{end+1} = filename;
res_ioq_cot.draw(props{:})
th = title([title_ioq_cot, SPC], 'FontSize', FM*FONT_SIZE);
th.Position=POS;
export_fig(filename)
...
out_pan = fullfile(out_folder, 'ioq_cot.pdf');
